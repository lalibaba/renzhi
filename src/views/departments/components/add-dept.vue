...
export default {
  ...
  data() {
    ...
    const checkCodeRepeat = async(rule, value, callback) => {
      ...
      if (this.formData.id) {
        // 排除正在编辑的部门
        // isRepeat = depts.filter(item => item.id !== this.treeNode.id).some(item => item.name === value)
        isRepeat = depts.some(
    ...
    const checkNameRepeat = async(rule, value, callback) => {
      ...
      // 编辑时的校验
      if (this.formData.id) {
        // [编辑时的校验] 如何去找技术部所有的子节点
        // isRepeat = depts
        //   .filter((item) => item.pid === this.treeNode.pid && item.id !== this.treeNode.id
        //   ).some((item) => item.name === value)
        // 方法二
        isRepeat = depts.some(item => item.id !== this.treeNode.id && item.pid === this.treeNode.pid && item.name === value)
      } else {
        // [新增时的校验] 如何去找技术部所有的子节点
        isRepeat = depts
      ...
      isRepeat
        ? callback(new Error(`同级部门${this.formData.id ? '' : '下'}已经有${value}的部门了`))
        : callback()
    }
    return {
  ...
  methods: {
    ...
    async btnOK() {
      try {
        await this.$refs.formDept.validate()
        this.loading = true
